...
uint32_t hash_murmur2_U8(const uint8_t* pdata, size_t length, uint32_t initval)
{
    ...
    uintptr_t h = (sizeof(uintptr_t) == 8) ? initval ^ length * m : initval ^ length;

    const uintptr_t* data = (uintptr_t*)pdata;
...
uint32_t hash_computeU8(const uint8_t* data8, size_t length, uint32_t initval)
{
     ...
     */
    switch (len) {
    case 11: /* 2 uint32 + 1 uint16 + 1 uint8 */ a += mem_get<uint8_t>(((uint16_t*)data) + 5); HASH_MIX(a, b, c);
