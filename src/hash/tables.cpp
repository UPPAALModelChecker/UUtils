 ...
 * same info.
 */
static inline size_t hash_countCollisions(const SingleBucket_t* root0)
{
    size_t count = 0;
    for (const SingleBucket_t* root = root0; root != nullptr; root = root->getNext()) {
 ...
 */
static inline size_t hash_countCollisions(const DoubleBucket_t* root0)
{
    size_t count = 0;
    for (const DoubleBucket_t* root = root0; root != nullptr; root = root->getNext()) {
        // Check not already counted
        bool ok = true;
...
void rehash(SingleBucket_t*** oldTablePtr, uint32_t* maskPtr)
{
    ...
    *oldTablePtr = newBuckets;  // set new table
    ...
    std::fill(newBuckets + oldSize, newBuckets + (oldSize * 2), nullptr);
    ...
    // stats
#if defined(SHOW_STATS) && !defined(NDEBUG)
    std::cerr << nbBuckets << '/' << oldSize << " true collisions=" << oldTrueCollis << "\nOld collisions("
              << oldCollisions << ')';
    size_t size = lenStat.size();
    for (size_t j = 0; j < size; ++j) {
        if (lenStat[j])
            std::cerr << " [" << j << "]=" << lenStat[j];
    }
    ...
    std::cerr << "\nNew collision(" << newCollisions << ") average=" << average << std::endl;
#endif
}
...
void rehash(DoubleBucket_t*** oldTablePtr, uint32_t* maskPtr)
{
    ...
    *oldTablePtr = newBuckets;
    ...
    nbEntries = nbBuckets - newCollisions;
    average = nbEntries ? ((100 * nbBuckets) / nbEntries) / 100.0 : 0.0;
    std::cerr << "\nNew collisions(" << newCollisions << ") average=" << average << std::endl;
#endif
}
