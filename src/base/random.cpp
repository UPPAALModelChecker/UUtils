...
uint32_t RandomGenerator::uni(const uint32_t max) { return std::uniform_int_distribution<uint32_t>{0, max}(s->rnd); }
...
double RandomGenerator::uni_r(const double max) { return std::uniform_real_distribution<double>{0.0, max}(s->rnd); }
