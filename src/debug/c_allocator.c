...
int32_t* debug_malloc(size_t intSize, void* debugAllocations)
{
    ...
    debug_allocation_t* entry =
        (debug_allocation_t*)malloc(sizeof(debug_allocation_t) + (intSize + 2) * sizeof(int32_t));
    if (!entry) {
    ...
    /* Write magic numbers */
    data[0] = (intptr_t)&data[1];  // FIXME: works only in 32bit architectures
...
void debug_free(void* ptr, size_t intSize, void* debugAllocations)
{
    ...
    fprintf(stderr, "Free aborted: data at %p not allocated!\n", ptr);
}
...
void debug_destroyAllocator(allocator_t* debugAllocator)
{
    ...
    if (nbLeaks) {
        ...
        fprintf(stderr, GREEN(BOLD) "No leak detected (allocator_t)." NORMAL "\n");
    }
#endif
