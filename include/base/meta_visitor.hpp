...
template <typename Visitor>
constexpr auto is_visitor_v = is_visitor<Visitor>::value;

/** Primary declaration for types accepting visitor trait (specialize to enable support): */
...
template <typename V, typename D, typename = std::enable_if_t<is_visitor_v<std::remove_reference_t<V>>>>
V& operator<<(V&& v, const D& d)
{
    v.visit(d);
    return v;
}
/** Helper input operator for any visitor visiting any data: */
template <typename V, typename D, typename = std::enable_if_t<is_visitor_v<std::remove_reference_t<V>>>>
V& operator>>(V&& v, D& d)
{
    v.visit(d);
