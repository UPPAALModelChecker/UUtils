...
public:
     ...
     */
    bool isSamePointerAs(const pointer_t<T>& other) const { return begin() == other.begin() && size() == other.size(); }
    ...
    /** @return a hash value.
     */
    uint32_t hash() const { return hash_computeU32((uint32_t*)data, intSizeOfA(T, capa), capa); }
